# STT 품질 평가 방법

## 1. 왜 시험지가 먼저 필요한가

STT 모델이나 prompt를 바꾼 뒤 전사문 몇 개를 눈으로 보는 것만으로는 개선 여부를
알 수 없다. 쉬운 음성만 좋아지고 작은 목소리나 필러가 많은 음성은 나빠질 수 있다.

따라서 모델을 바꾸기 전에 항상 같은 문제를 풀게 하는 “STT 시험지”를 만든다.
정답을 만드는 표기·검수 기준은 [`gold-transcript-guide.md`](gold-transcript-guide.md)의
`gold-v1-rc1` 규범안을 참고한다. 현재는 실제 음성 파일럿 검증 전이며, 확정판과 구별한다.

```text
고정 음성 + 사람이 검수한 정답 전사문
                  ↓
기존 설정과 새 설정이 같은 시험을 봄
                  ↓
정확도·필러·환각·시간·비용을 함께 비교
```

## 2. 평가 단위

한 평가 sample은 다음 정보를 가진다.

```json
{
  "id": "clean-filler-001",
  "audioPath": "samples/clean-filler-001.webm",
  "referenceTranscript": "Um, I I usually go there on weekends.",
  "tags": ["clean", "filler", "repetition"],
  "durationSeconds": 5.8,
  "language": "en",
  "source": "synthetic",
  "expectedBehavior": "transcribe"
}
```

`referenceTranscript`는 두 사람이 원본 음성을 독립적으로 듣고 합의한 완전한
단어열이다. 전체 verbatim Gold는 별도 비공개 파일에 조각·사건과 함께 보존하고,
`referenceTranscript`는 현재 채점용으로 추출한 표현이다. 합의되지 않은 발화는
보류하고, 조각·단어 내부 중단·혼용 언어·복수 화자 자료는 수동 도전 자료로 분리한다.
분리한 자료의 수와 이유도 보고한다. 이 분류는 현재 CLI가 자동으로 적용하지 않는다.
주 단어 평가에서도 필러·완전 반복·false start의 완전한 단어·문법 오류는 삭제하지 않는다.

## 3. 평가 자료 구성

초기 자료는 최소한 다음 범주를 골고루 포함한다.

| 범주 | 확인하려는 문제 |
|---|---|
| 깨끗한 보통 속도 | 기본 정확도 |
| 작은 목소리 | 낮은 신호에서의 누락 |
| 일정한 배경 소음 | 소음을 단어로 만드는지 |
| 빠른 발화 | 단어 합침과 누락 |
| 느린 발화 | 과도한 문장 분리와 기다림 |
| 긴 첫 침묵 | 시작 부분 손실과 latency |
| 긴 중간 pause | VAD와 chunk 경계 |
| 필러 다수 | `um`, `uh` 보존 |
| 연속 반복 | `I I`, `the the` 보존 |
| false start | 다시 시작한 문장 보존 |
| 틀린 문법 | 자동 교정 여부 |
| 틀린 어휘 | 의도 추측과 교체 여부 |
| 한국식 고유명사 | 이름과 장소 인식 |
| OPIc 주제 어휘 | 집, 공원, 여행, 역할극 어휘 |
| 아주 짧은 답변 | 3초 안팎 입력 |
| 긴 답변 | 120초에 가까운 입력 |
| 무음 | 빈 결과와 오류 처리 |
| 비음성 잡음 | 환각 문장 생성 여부 |
| 손상된 파일 | 안전한 입력 오류 처리 |

한 사람의 목소리만 사용하면 그 사람에게 과하게 맞춘 결과가 된다. 가능하면 성별,
음높이, 말하기 속도, 영어 숙련도가 다른 화자를 포함한다.

## 4. 개인정보와 fixture 정책

- 운영 사용자의 녹음을 자동으로 평가 자료에 넣지 않는다.
- 합성 음성은 pipeline 테스트에는 사용할 수 있지만 비원어민 발화 품질 평가를 대신할
  수 없다.
- 실제 화자 자료는 명시적 동의, 사용 목적, 보존 기간, 삭제 방법을 기록한다.
- 이름, 학교, 회사, 주소 등 개인 식별 정보가 포함된 sample은 사용하지 않는다.
- 공개 저장소에 넣을 수 없는 오디오는 별도 접근 제어 저장소에 두고 manifest만 관리한다.
- Git에 큰 binary를 직접 계속 추가하지 않는다. 필요하면 Git LFS 또는 전용 저장소를
  선택하고 ADR로 기록한다.

## 5. 텍스트 정규화 규칙

WER 계산 전 비교에 불필요한 표기 차이를 정규화한다.

현재 코드의 정규화:

1. NFKC 문자 정규화 후 소문자로 변환
2. 곡선 apostrophe `’`를 `'`로 치환
3. `[a-z0-9]+(?:'[a-z0-9]+)*` 패턴으로 단어 추출

숫자·축약형·약어의 의미 동등성은 처리하지 않는다. `21`과 `twenty one`,
`I'm`과 `I am`은 다른 단어열이며, 한글은 현재 단어 추출에서 빠진다.
Gold 사건 태그나 조각을 그대로 넣으면 설명과 조각도 단어로 세므로 주의한다.
이 한계 때문에 현재 WER에는 출력 표기 차이가 섞일 수 있다.

정규화하면 안 되는 것:

- 필러 제거
- 반복 단어 제거
- 문법 교정
- 숫자와 단어 표기를 근거 없이 같은 것으로 간주
- false start 삭제

원문 보존 품질을 측정해야 하므로 평가 목표인 차이를 지워서는 안 된다.

## 6. 핵심 품질 지표

### 6.1 Word Error Rate

```text
WER = (대체한 단어 + 빠뜨린 단어 + 추가한 단어) / 정답 단어 수
```

낮을수록 좋다. 전체 정확도를 한 숫자로 비교하기 좋지만 OPIc에 중요한 필러와
문법 오류 보존 여부를 따로 설명하지 못한다.

### 6.2 삭제율

정답에 있었지만 STT가 빠뜨린 단어 비율이다. 작은 목소리, 빠른 발화, 문장 끝이 자주
누락되는지 확인한다.

### 6.3 삽입률과 환각률

사용자가 말하지 않았는데 STT가 추가한 단어의 비율이다. 무음·잡음에서 완성된
문장을 만들면 치명적인 실패로 별도 집계한다.

### 6.4 필러 보존율

```text
올바르게 보존한 필러 수 / 정답 필러 수
```

현재 자동 지표는 `um/uh/er/ah/hmm` 종류별 개수만 비교한다. 필러가 없는 sample은
이 지표의 분모에서 제외한다. 위치와 담화 기능은 수동 검토해야 한다.
`uh-huh`, `mm-hmm` 같은 응답 소리도 분리된 일부 토큰이 필러로 집계될 수 있다.

### 6.5 반복 보존율

현재 자동 지표는 바로 이어진 같은 단어의 **개수**만 측정한다. false start의
완전한 단어는 Gold와 WER에 남지만, false start 경계의 보존 여부는 아직 별도
지표로 측정하지 않는다. 일반 WER가 같아도 반복을 지운 모델은 OPIc 유창성
분석에 불리할 수 있다.

### 6.6 오류 보존율

목표 지표다. 사람이 실제로 말한 문법·시제·어휘 오류를 STT가 교정하지 않고 그대로
남겼는지 확인한다. 현재는 전체 WER와 수동 검토로 확인하며 별도 자동 지표는 없다.

### 6.7 핵심 어휘 정확도

목표 지표다. 장소명, OPIc 주제 핵심어처럼 의미에 큰 영향을 주는 단어를 별도 목록으로
표시해 recall을 계산하려는 계획이며 현재 자동 구현은 없다. 비공개 실명은 자료에서 제외한다.

### 6.8 빈 전사율

실제 음성이 있는 sample에서 빈 문자열 또는 공백만 반환한 비율이다.

### 6.9 비음성 환각률

무음이나 비음성 잡음 sample에서 실제 단어나 문장을 생성한 비율이다. 가능하면 0이어야
한다.

## 7. 운영 지표

품질만 좋고 너무 느리거나 비싸면 제품에서 사용할 수 없다.

| 지표 | 의미 |
|---|---|
| latency p50 | 보통 사용자가 기다리는 시간 |
| latency p95 | 느린 요청이 기다리는 시간 |
| 성공률 | 정상 응답 비율 |
| timeout 비율 | 제한 시간을 넘긴 요청 비율 |
| retry 비율 | rate limit 등으로 다시 호출한 비율 |
| 오디오 1분당 비용 | 모델 비교를 위한 비용 기준 |
| 입력 1분당 token/usage | 비용 변화 원인 분석 |
| 형식별 실패율 | WebM, MP4 등 호환성 확인 |

운영 로그에는 sample ID 같은 비민감 식별자만 사용하고 오디오 또는 전사 내용을
기본 로그에 포함하지 않는다.

## 8. 실험 단위

하나의 실험에서는 가능한 한 한 변수만 바꾼다.

좋은 실험:

- 같은 모델에서 prompt A와 B 비교
- 같은 prompt에서 mini 모델과 상위 모델 비교
- 같은 설정에서 alias와 고정 snapshot 비교
- 같은 모델에서 chunking 없음과 자동 VAD 비교

나쁜 실험:

- 모델, prompt, bitrate, VAD를 한꺼번에 변경

여러 변수를 동시에 바꾸면 어떤 변경이 결과를 만들었는지 알 수 없다.

## 9. 실험 기록 형식

각 실행은 다음 정보를 남긴다.

```json
{
  "experimentId": "2026-09-mini-prompt-b",
  "model": "모델 또는 snapshot ID",
  "promptVersion": "transcription-v2",
  "audioConfig": {
    "chunking": "none",
    "language": "en"
  },
  "datasetVersion": "stt-eval-v1",
  "metrics": {
    "wer": 0.0,
    "fillerRecall": 0.0,
    "repetitionRecall": 0.0,
    "hallucinationRate": 0.0,
    "latencyP50Ms": 0,
    "latencyP95Ms": 0,
    "estimatedCostPerAudioMinute": 0.0
  }
}
```

실제 전사문은 민감 정보가 없는 승인된 평가 자료에 대해서만 별도 결과 파일에 저장한다.

## 10. 채택 판단

새 설정은 평균 WER 하나만 좋아졌다고 채택하지 않는다.

필수 gate:

- 비음성 환각률이 악화되지 않음
- 필러 보존율이 악화되지 않음
- 반복과 오류 보존율이 악화되지 않음
- 중요한 화자·환경 범주에서 치명적인 regression이 없음
- latency와 비용이 제품 허용 범위 안에 있음
- 동일 설정으로 반복 실행했을 때 결과가 허용 범위 안에서 재현됨

초기 수치 목표는 baseline을 측정한 뒤 정한다. 근거 없이 임의 목표를 먼저 정하지 않는다.

## 11. 평가 자료 분리

자료가 충분해지면 다음 세 묶음으로 나눈다.

- **development set**: prompt와 설정을 조정할 때 사용
- **validation set**: 후보 설정을 비교할 때 사용
- **test set**: 최종 선택 전에만 사용

같은 화자의 비슷한 문장이 여러 묶음에 들어가면 실제보다 성능이 좋아 보일 수 있다.
가능하면 화자 단위로 분리한다.

## 12. 첫 번째 평가 도구 구현 순서

1. 민감 정보가 없는 sample manifest schema를 정의한다.
2. 정답 전사문 정규화 규칙에 단위 테스트를 추가한다.
3. 단어 정렬과 WER 계산기를 구현한다.
4. 필러와 반복 보존 지표를 구현한다.
5. 무음·비음성 결과 판정을 구현한다.
6. provider 설정별 실행 adapter를 구현한다.
7. latency와 usage를 수집한다.
8. JSON 결과와 사람이 읽는 Markdown 요약을 생성한다.
9. CI에서는 비용 없는 고정 결과 fixture로 계산기만 테스트한다.
10. 실제 외부 API 평가는 명시적으로 실행하는 별도 명령으로 둔다.

## 13. 완료 조건

- 같은 dataset과 설정으로 결과를 다시 만들 수 있다.
- metric 계산이 자동 테스트로 검증된다.
- 모델과 prompt 변경 PR에 이전/이후 비교표가 포함된다.
- 음성과 전사문이 일반 애플리케이션 로그에 노출되지 않는다.
- 품질, latency, 비용 중 어느 하나를 숨기고 “개선”이라고 표현하지 않는다.

## 14. 현재 구현 상태

현재 저장소에는 다음 오프라인 평가 기반이 구현되어 있다. 중심 파일은
`backend/app/stt_benchmark/evaluation_engine.py`다. 이 파일은 모델을 호출하거나
JSON을 읽지 않고 전달받은 데이터셋과 실행 결과만 채점한다.

- `backend/app/stt_benchmark/contracts/`: 평가 manifest·실행 결과·성적표 계약
- `backend/app/stt_benchmark/metrics/`: WER, 필러, 연속 반복어, 비음성 단어 계산
- `backend/app/stt_benchmark/evaluation_engine.py`: sample별 계산과 전체 성적표 조정
- `backend/app/stt_benchmark/transcription_runner.py`: 주입받은 STT provider로 평가 음성 실행
- `backend/app/stt_benchmark/interfaces/cli/`: JSON 입출력과 명령행 인자 처리
- `backend/app/stt_benchmark/evaluation_orchestrator.py`: 여러 독립 평가기의 병렬 실행과
  실패 격리
- `backend/app/stt_benchmark/evaluators/`: Custom, JiWER, Nyra, SCTK, HF Evaluate,
  MeetEval adapter
- `backend/app/stt_benchmark/storage/`: 평가기별 native 결과와 최소 실행 인덱스 저장
- `backend/app/evals/transcription_*.py`: 기존 import와 실행 명령을 위한 호환 경로
- `backend/evals/transcription/`: 공개 예시와 로컬 자료 배치 안내
- `docs/ai/gold-transcript-guide.md`: Gold 작성·검수 규범안 (`gold-v1-rc1`, 파일럿 전)

새 통합 명령은 아래처럼 실행한다. 기존 `app.evals.transcription_cli`와
`app.evals.transcription_execute_cli` 명령도 같은 구현으로 계속 동작한다.

```powershell
python -m app.stt_benchmark evaluate --manifest <manifest.json> --run <run.json>
python -m app.stt_benchmark evaluate-engines --manifest <manifest.json> --run <run.json> --output-dir <results-dir> --evaluation-run-id <id> --evaluators custom,jiwer
python -m app.stt_benchmark transcribe --manifest <manifest.json> --output <results/run.json> --experiment-id <id>
```

`evaluate`는 기존 통합 성적표 의미와 호환성을 유지한다. `evaluate-engines`는 평가기끼리
결과를 합치거나 순위를 만들지 않고 각 native 결과를 별도 보존한다. 평가기별 설치 방법,
입력 조건, 라이선스와 현재 검증 상태는
[`stt-multi-engine-evaluation.md`](stt-multi-engine-evaluation.md)를 따른다.

아직 구현하지 않은 항목:

- 동의받은 실제 비원어민 평가 음성 세트
- OpenAI 등 다른 STT 공급자의 실제 실행 adapter와 설정 비교 게이트
- fragment·위치 기반 filler·false start·pause 자동 Custom 지표
- PostgreSQL 공동 저장소, 연구 API, 작업 worker와 Docker 배포 구성
- false start와 사람이 표시한 문법·어휘 오류의 보존 지표
- Gold 주석 파일의 자동 검증·채점용 추출·평가 묶음 필터, 불명확 구간 제외, stutter·pause 채점
- 실제 API usage를 이용한 오디오 1분당 비용 집계
- 여러 평가기 결과의 자동 비교·종합·순위·보고서(현재 요구 범위에서 의도적으로 제외)

현재 CLI는 이미 저장한 모델 실행 결과를 평가하며 외부 API를 호출하거나 비용을
발생시키지 않는다. 실행 방법은
`backend/evals/transcription/README.md`에 기록되어 있다.
